   ...
   /**
    ...
    request: function(clickPosition, options) {
        ...
        if(this.drillDown === false) {
			UOPA = this.url;
			this.url='http://mapa.ign.gob.ar/geoserver/wms';
            ...
            for(var i=0, len=layers.length; i<len; i++) {
                ...
                if(url in services) {
                    ...
                    this._numRequests++;
					/**
					 * Si la capa que estoy por pedir GetFeatureInfo
					 * es IGN.CACHE entonces, consulto a IGN.SERVER
					 */
					if (url == IGN.CACHE) {url = IGN.SERVER;}
                    services[url] = [layer];
                }
            }
            var layers;
...
//		debugger;
     ...
     */
    triggerGetFeatureInfo: function(request, xy, features, text) {
    ...
    handleResponse: function(xy, request) {
        ...
        if (this.drillDown === false) {
            this.triggerGetFeatureInfo(request, xy, features, doc);
        } else {
            this._requestCount++;
            this._features = (this._features || []).concat(features);
	    this._text += doc;
            if (this._requestCount === this._numRequests) {
                this.triggerGetFeatureInfo(request, xy, this._features.concat(), this._text); 
